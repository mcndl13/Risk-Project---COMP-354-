...
bool CheaterPlayerStrategy::issueOrder(Player *player, Deck* deck, Map* territoriesMap, const vector<Player*> gamePlayers){
    CheckForBonusCard(player,deck);
    for(auto it : CheaterPlayerStrategy::toAttack(player)){
        player->add_new_player_territory(it);
        cout<<"\nCheater takes a territory "<<it->getName()<<endl;
    }

    return true;
}

vector<Territory*> CheaterPlayerStrategy::toAttack(Player *player){//push every adjacent territory that does not belong to cheater in the list
...
vector<Territory*> CheaterPlayerStrategy::toDefend(Player *player){//put territories that are adjacent to enemy territory into list
    vector<Territory*> listToReturn;
...
vector<Territory*> BenevolentPlayerStrategy::toAttack(Player *player) {
    vector<Territory*> temp = vector<Territory*> ();
    return temp;
}//play defensive

vector<Territory*> BenevolentPlayerStrategy::toDefend(Player *player){//put territories that are adjacent to enemy territory into list
    vector<Territory*> listToReturn;
    for(auto it : player->getPlayerTerritories()){
        for(auto _it : it->getAdjacencies()){
            if(!(player->ownsTerritory(_it))){
...
bool BenevolentPlayerStrategy::issueOrder(Player *player, Deck* deck, Map* territoriesMap, const vector<Player*> gamePlayers) {
    CheckForBonusCard(player,deck);//probably will not give card here
    weakest_territory = set_weakest_territory(player);
    //random number
    random_device rd;
    mt19937 gen(rd());
    uniform_real_distribution<int> dis(1, player->getReinforcementPool());
    ...
    for(auto it:  stress_list){
        ...
        for(auto _it : currTerr->getAdjacencies()){
            if(player->ownsTerritory(_it)){
                auto __it = find_if(stress_list.begin(), stress_list.end(),
                           [&_it](const auto& pair) {
                               return pair.first == _it;
                           });//to find Territory* in a vector<pair<Territory*,int>>
                if (__it != stress_list.end()) {//if found
                    //cout << "Territory found in the  " << __it->first->getName() << endl;
                    if(maxStress<__it->second){//if the stress is bigger there, target will be it
                        maxStress = __it->second;
                        tarTerr = __it->first;
                    }
                } else {
                    cerr << "Territory not found in the weak list, this is in a loop inside issureOrder in PlayerStrategies." << endl;
                }
            }
        }
    ...
    for(int i =0; i<handOfCard.size()-1;i++){//loop through cards
        if(handOfCard[i]->getCardType() == "Diplomacy"){//check if we have airlift card
            if(stress_list.size()>0){//if yes, check if we have more than one
                ...
                pair<Player*, int> playerWithPower = make_pair(nullptr, 0);//fisrt is player and second is power
                for (auto it : weakest_territory->getAdjacencies())//check the weakest's adjacencies
                {
                    if(playerWithPower.second<it->get_number_of_armies()){//to get the biggest army number territory and 
                        playerWithPower = make_pair(it->get_owning_player(), it->get_number_of_armies());
                    }
                }
                ...
                player->addOrder(negotiateOrder);//still need to be excuted in main function with a loop
                return;
            }
        }
    }
    ...
    for(int i =0; i<handOfCard.size()-1;i++){//loop through cards
        if(handOfCard[i]->getCardType() == "Airlift"){//check if we have airlift card
            if(stress_list.size()>1){//if yes, check if we have more than two territories
                ...
                player->addOrder(airlift);//still need to be excuted in main function with a loop
                return;
            }
        }
    }
...
Territory* BenevolentPlayerStrategy::set_weakest_territory(Player* player){
    pair<Territory*,int> stressOnTerr =make_pair(nullptr,0);
    int tempStress = 0;
    for(auto it: BenevolentPlayerStrategy::toDefend(player)){//looping todefend list
            ...
            if(tempStress>stressOnTerr.second){
                stressOnTerr = make_pair(it,tempStress);
            }
...
vector<pair<Territory*, int>> BenevolentPlayerStrategy::set_List_of_weakest_territories(Player* player){
    vector<pair<Territory*, int>> stressList;
    int tempStress = 0;
    for(auto it: player->getPlayerTerritories()){//looping owned list
            ...
            stressList.push_back(make_pair(it,tempStress));
            tempStress = 0;//reset stress for next loop
    }
    ...
    sort(stressList.begin(), stressList.end(), compareBySecond);
