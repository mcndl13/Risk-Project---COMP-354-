...
// Card
    ...
    */
    void Card::play(Hand* hand, int index, Deck* deck, Player * player){
        ...
        Order* new_order = new Order(hand->getCardInHand(index)->getCardType());

        player->issueOrder(new_order);
...
// Deck
    ...
    */
    Card* Deck::draw() {
        ...
        int index = (rand() % (int) deck_cards.size() ) + 1;
...
// Hand
    ...
    Card* Hand::getCardInHand(int index){return cards_in_hand.at(index);}
...
// Player
    ...
    // Add an order to (player_orders), the collection of order made by a player
    void Player::issueOrder(Order* order){
        player_orders.push_back(order);
    }
...
// Order
    Order::Order(const string& inDescription){
        this-> description = inDescription;
        cout << "The " << description << " order has been initiated.";
    }

    //Printing a collection of orders
    ostream& operator<< (ostream& out, const vector<Order*> orders) {
        out << "[ ";
        for (int i = 0; i < orders.size(); i++) {
            out << orders[i]->description;
            if (i != orders.size() - 1) out << ", ";
        }
        out << " ]\n";

        //The output has the form [ order1, order2, order3 ]
        return out;
    }
